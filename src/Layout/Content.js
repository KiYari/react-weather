...
class Content extends React.Component {
  ...
  data_scrapper = (weather) => {
    ...
    this.state.temperature_fluc = Math.floor(weather[8] - 273) + 'C' + ' - ' + Math.ceil(weather[9] - 273) + 'C'
    this.state.cond_icon = 'http://openweathermap.org/img/wn/' + weather[10] + '@2x.png'
    this.state.icon = <img style={{
        width: '4vh',
        height: '4vh'
      }} src={this.state.cond_icon}/>
    this.forceUpdate();
  }
        ...
        <Grid item="item" xs={12}>
          <Paper className={classes.contenter}>
            <div>
              ...
              <br/>
              Current weather(k) = {this.state.Fweather}
              <br/>
              Current weather = {this.state.Dweather}
              <br/>
              Current conditionals = {this.state.conditionals}
              {this.state.icon}<br/>
